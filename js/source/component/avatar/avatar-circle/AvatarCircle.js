...
function AvatarCircle(props) {
  ...
  return (
    <Element  className={`user-avatar-circle ${!props.data.rank ? 'no-stats' : ''} ${noBorderClasses} ${props.modifiers || ''}`}
...

    {
      props.data.rank &&
        <React.Fragment>
          ...
          <CircleProgressBar size="regular" fillAmount={props.data.rank.current / props.data.rank.total} />
          {/* USER AVATAR CIRCLE PROGRESS */}
...
export { AvatarCircle as default };